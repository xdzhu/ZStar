...
|---|---|---|---|
| A0.1 R1 参考平衡结构 | **完成**；HF job `27697265`，node14，32 MPI × 1 OMP，813 s | 固定输入协议 | `cell-relax` 已收敛并产生 `STRU_ION_D`；今后固定 `force_thr=1e-4 eV/Å`、`stress_thr=0.5 kbar`、`scf_thr=1e-8`、`relax_nmax=100`；此前以 `0.1 kbar` 收敛的结果无需重跑 |
...
3. **3C-SiC 与 diamond Si：** 3C-SiC 是非中心对称 `F-43m` 压电体，验证独立
   `e14`，不得再把它误写成零压电 control；另用中心对称 `Fd-3m` diamond Si
   验证压电严格为零与弹性结果。
4. **tetragonal 极性体系：** 在前述六方/立方控制通过后，选 PbTiO3 或 HfO2 的
   一个已绝缘、文献充分的体系验证低对称/极性情形。
5. **PZT 构型研究：** 首个模型固定为 `PbZr0.5Ti0.5O3`、[001] 1:1 B 位层状有序、
   四方 `P4mm` 10 原子胞；其结果只代表该显式构型，不代表无序 PZT、MPB 平均体或
   单斜相。随后再独立比较 `Cm` 和其他 B 位构型。
6. **orthorhombic/低对称体系：** 最后只选一个有匹配理论文献的绝缘体，验证一般
