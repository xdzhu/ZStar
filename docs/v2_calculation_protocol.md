...
“自动选最优幅度”。`±0.0025/±0.005/±0.01` 仅是 v2 研发者在代表性 benchmark 上一次性
完成的 workflow qualification；有限差分弹性文献中 `±0.5%/±1%` 是常见的线性审计组合。
这项内部证据足以支持固定 production 默认值后，普通用户只运行 `±0.005` 的完整中心
差分 ensemble。
