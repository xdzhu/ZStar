...
超高精度不是生产默认值。生产档用于一个已完成收敛预检材料的完整 response ensemble；
验证档只用于首个 benchmark、异常分量、cutoff/k-point/幅度审计和论文冻结前复核。二者
都不是“默认即正确”：是否升级由张量对 cutoff、k 网格和应变幅度的稳定性决定。
...
生产候选幅度默认是 `±0.005`（`±0.5%`），但不是普适物理常数。每个新的材料/后端
组合先在验证档审计 `±0.0025`、`±0.005`、`±0.01`；报告实际应变、branch shift、
rank、condition number 与拟合残差。有限差分弹性文献中 `±0.5%/±1%` 是常见的线性
审计组合。只有三个幅度中存在稳定线性窗口，才把其中信噪比最好的一个写入该材料的
production provenance；软模、近相变或 branch 不连续体系必须缩小并重新审计，不能
机械沿用 `0.5%`。
...
- 生产档不是降低科学标准：每个新材料仍要有一个 verification 子集、幅度扫描及与匹配
  泛函理论文献的对照；无法通过者只能输出 `research/conditional`，不能称为 benchmark。
