...
3. 响应导数必须由**实际序列化的**位移/应变向量计算，默认采用中心差分；只有用户
   显式选择时才采用前向差分，并提示 O(h) 截断误差、推荐 O(h²) 的中心差分。
   名义步长只作为任务生成参数。
...
未通过并由维护者区分数值噪声与非线性。普通用户默认运行 `±0.005` 的完整中心差分
ensemble；显式前向模式采用相同固定幅度的 `0,+0.005`，不自动改变精度或扫描幅度。

### 显式差分算法选择

Python API `prepare_abacus_strain_ensemble(..., method="central")` 与
`fit_finite_difference_document(..., method="central")` 默认中心差分。
用户明确传入 `method="forward"` 才启用前向差分；准备和拟合时都会发出
`UserWarning`，解释光滑响应下前向 O(h)、中心 O(h²) 的截断误差并推荐中心。
误差阶数不是总误差保证：电子/离子收敛、极化积分与分支匹配仍必须通过检查。

现有开发工具（不是新增正式 `zstar piezo` CLI）均支持 `--method central|forward`：

```text
python tools/collect_v2_piezo_case.py CASE_ROOT --method forward --output CASE_ROOT/results-forward
```

输入准备工具 `tools/prepare_v2_piezo_case.py` 同样可显式追加 `--method forward`。
前向准备集合只有参考点和正向应变点；完整六分量是 7 个几何，默认中心为 13 个。
已有完整中心集合也可用于前向后处理，不增加计算；应使用独立输出目录保留原结果。
混合方向以首个显著非零工程应变分量为正作为规范方向，忽略序列化舍入量。
中心模式检查实际正负向量唯一配对，缺点或幅度不匹配时明确失败，绝不自动降级；
前向模式仍要求所求张量重建满秩。

结果文档、拟合张量 provenance、准备 manifest 与结果 summary 保存算法、截断误差阶数
和推荐项；结果文档还保存实际使用的 stage 名称与源索引。通用研究拟合函数
`fit_response_document` 保留任意采样拟合能力，不将任意单边样本冒充中心差分。
上述选择仅影响 v2，不改动 v1 已发布接口或默认值。
