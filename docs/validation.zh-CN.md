...
[构建验收](https://github.com/xdzhu/ZStar-Polarization-BEC-Dielectric-Spectroscopy/actions/runs/34056725679)、
[发布验收](https://github.com/xdzhu/ZStar-Polarization-BEC-Dielectric-Spectroscopy/actions/runs/34056874177)。
[GitHub Release](https://github.com/xdzhu/ZStar-Polarization-BEC-Dielectric-Spectroscopy/releases/tag/v0.3.1) 与
...
- 调度脚本：在直连计算环境检查 shell 与 Torque/PBS，并在独立 Slurm 集群
  检查 Slurm 脚本和运行环境。
- 最终本地回归：Python 3.10 环境下 205 个测试全部通过。
...
当前源码通过全部 205 项测试。公开的立方 BaTiO3 案例也成功执行
`zstar bec post`，并复现归档的代表性电荷：
...
另一个对称后端基准从结构优化开始，对 3C-SiC/PBE 和四方 HfO2/PBEsol
分别完成 ABACUS + PYATB 与 VASP 全流程。SiC 光学三重简并模相差 0.477%；
HfO2 的 15 个光学模逐模 MAE 为 4.314 cm^-1。完整的分阶段核时和比较边界见
[谱学后端基准](spectroscopy_backend_benchmark.zh-CN.md)。
...
实机验证。可复现的基线采用 PBE、100 Ry 截断、9 au 数值原子轨道、0.01 Angstrom
...
同一套中心差分 cube 流程也完成了 HSE 计算，采用 `scf_thr = 1e-7`；精简记录保留在
分子案例目录中。

| 分子 | 原子 | ABACUS + PYATB PBE GAPT (e) | ABACUS + PYATB HSE GAPT (e) | CP2K 偶极，0.005 Angstrom (e) | CP2K 原生 APT (e) |
| --- | --- | ---: | ---: | ---: | ---: |
