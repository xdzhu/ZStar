...
0.3.3 发布候选版通过 393 项软件包测试、30 个安装后 CLI 检查，以及全部 40 个
公开案例的 dry-run。独立安装的 wheel 中运行时版本与发行元数据一致，可发现随包
提供的 agent skill，且 PyPI 制品不包含案例库。最终稿件引用审计覆盖 76 条实际
引用记录，标题、DOI 与 URL 元数据完整，并与首次引用顺序一致。

0.3.3 使 agent skill 及其预检查与公开的 Unified 响应工作流保持一致，支持从
ZStar 配置解析计算软件路径，并更新中英文手册及谱学图中的最终文献编号。已有数值
结果未改变。
