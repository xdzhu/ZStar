...
| `relaxed_response.py` | 从带显式轴/单位/ion-state/proper 标记的 `ResponseDocument` 重排 BEC、收缩 Lambda，并追加 `piezoelectric_internal`/`piezoelectric_relaxed`；同时从 clamped `C`、IFC `Phi` 和 `Gamma` 组装 relaxed `C`、内部校正和 equilibrium Lambda；体积只来自显式 SI 参数或参考晶格 | 猜测 BEC/IFC 轴顺序、单位、Berry branch 或 clamped-ion 输入 |
