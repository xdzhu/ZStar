...
ABACUS 当前输入文档明确把 `press1/press2/press3` 的正值定义为压缩外应力，且应力
单位为 kbar；这为后续符号审计提供了软件依据，但并不自动等价于 `TOTAL-STRESS`
输出张量的完整符号约定，仍需受控算例确认（[官方输入文档](https://abacus.deepmodeling.com/en/latest/advanced/input_files/input-main.html#press1)）。
...
能量曲率提供了独立但尚未通过的交叉检查。以体积 `63.1121 Å³` 将
`FINAL_ETOT_IS` 的中心二阶差分换算为 GPa，正向法向分量在 `δ=0.001` 时约为
`320.386`，在 `δ=0.003` 时约为 `288.633`；剪切分量分别约为 `140.630` 和
`135.680`。法向幅度依赖约 10%，说明当前设置还没有达到可冻结的应变/截断/SCF
收敛水平；它也解释了为什么本轮 stress 拟合只能作为候选结果。
