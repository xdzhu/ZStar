...
| \\(Z^*\\Lambda\\) relaxed-ion 装配 | v1 提供 BEC/IFC 输入，但当前仓库没有可直接调用的高层装配 API | v2 新增显式长度/体积单位、轴序、边界条件和 acoustic-gauge 装配层；若外部 v1 已有实现，应改为适配器 | 新增装配 + 可复用输入 |
| 应力 work-conjugacy | v1 有原始应力/能量数据，但当前仓库没有 v2 的 engineering-Voigt 共轭契约 | v2 明确 tension/compression sign、engineering strain 与 tensorial stress Voigt 表示，并以能量曲率交叉核对 | 新增契约 + 兼容性审计 |
...
同理，v2 的 `relaxed_piezoelectric` 和 `relaxed_elastic` 没有重新发明物理定义，
但它们在当前仓库中确实是 v2 新增的高层装配代码，因为 v1 只提供了 BEC/IFC 等
基础输入。若用户已有仓库外的 v1 装配实现并且满足相同坐标、符号、单位和边界条件，
下一步应将其接成适配器并保留回归，而不是保留两套等价实现。

## 当前真正缺口

1. v1 结果到 v2 `ResponseDocument` 的 provenance 映射仍需冻结；
2. VASP 的应变力/应力 collector 和 POTCAR 输入一致性尚未验证；
3. stress work-conjugacy 需要确认是否存在仓库外的 v1 证据；若没有，则由 v2
   完成 engineering-Voigt 契约和能量曲率验证；
