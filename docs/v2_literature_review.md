...
数值对照也按这一构型边界分层。Sághi-Szabó 等的原始 GGA/LAPW+LO 工作给出
`[001]` planar `P4mm` 有序模型的 proper `e33=4.81 C/m²`；这是当前 PBE 计算最接近
的同构型半局域泛函锚点，但软件、基组、赝势和几何并不相同，且不能由摘要扩展出
未核实的完整 `e31/e15`。Eklund 和 Karttunen 随后用 CRYSTAL23、PBEsol0/TZVP
重新计算同类 planar `P4mm` 排序，报告 `d31=-5.0`、`d33=28.6`、`d15=70.8 pm/V`
以及 `e33=3.40 C/m²`（Phys. Chem. Chem. Phys. 27, 15920, DOI
`10.1039/D5CP01655J`）。后者提供明确的理论 `d33` 对照，但因使用25%精确交换的
PBEsol0，只作为同构型、跨泛函次级锚点。当前 ZStar PBE 完整张量结束后应分别列出
这两组比较，不能把实验陶瓷值或 PBEsol0 数值标成“PBE数据库复现”。
