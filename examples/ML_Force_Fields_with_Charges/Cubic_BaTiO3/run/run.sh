...
mkdir -p "$OUT_DIR"
zstar data qnep export --input "$RAW_JSONL" --annotations "$ANNOTATIONS_JSONL" \
  --output "$OUT_DIR/multiphase.xyz" --audit-output "$OUT_DIR/export.audit.json"
zstar data qnep check --input "$OUT_DIR/multiphase.xyz"

# Example phase-stratified composition.  Run GPUMD/qNEP training separately.
# zstar data qnep compose --base cubic_full.xyz --addition "$OUT_DIR/multiphase.xyz" \
