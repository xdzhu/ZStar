...
cu20 reboot interrupted one PYATB call; its elapsed cost is unrecorded. Listed totals cover completed calls, not the full billed total.

## BEC display and unified cost

`results/BEC_comparison/full_tensors.csv` contains all atoms in the manuscript
...
One interrupted force attempt on cu20 ended before its timing record was written;
the successful stages were preserved and the remaining 14 ran on cu25. The table
