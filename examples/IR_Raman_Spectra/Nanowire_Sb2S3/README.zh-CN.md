...
对比表同时加入了在上述 B3LYP-D3(BJ) 参考几何上计算的原生
VASP/PBE-D3(BJ) 结果。五类位点的轴向分量为
`(4.619,6.042,-4.120,-3.429,-3.112) e`，用于区分几何差异与其余泛函、
基组和实现差异。完整张量、不含 `POTCAR` 的输入、计时和来源记录位于
`results/BEC_comparison/vasp_reference_geometry/`。当前 ABACUS 3.10.0-LTS
生产二进制未启用 B3LYP 所需的 LibXC，输入检查阶段即拒绝了该泛函；因此没有将
失败的泛函解析冒充为计算结果。
...
Raman 绝对强度已验证。Sb2S3 的两套 Raman 结果覆盖相应的频率组，但归一化
相对权重不同，因此不宣称绝对强度的定量吻合。BN(9,0) 曾有一次任务中断，
计时表不包含无法追回的中断损耗。
