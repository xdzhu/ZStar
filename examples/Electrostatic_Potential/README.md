...
comparison. `SnS`, `SnSe`, and `SnTe` retain the broader
...
It is not a verified saddle point. The comparison uses a shared color scale for the
polar/nonpolar pair and tiles both maps 3x3 only when plotting.

Every case has the same layout:
...
and the exact command contract, but not the original raw cube or large
upstream SCF outputs. Their `run/README.md` records this boundary and the
