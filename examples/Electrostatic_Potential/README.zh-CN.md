...
薄层参考，`In2Se3` 展示面外极性薄层分析，`GeS` 提供强面内极性
对照；`SnS`、`SnSe` 和 `SnTe` 则保留更完整的面内方向静电势案例族。

`GeS_nonpolar` 新增固定离子的反演对称参考，附原始压缩 cube 与 SCF 证据。
在该目录运行 `bash run.sh` 即可离线重绘；`--calculate` 才会启动新 SCF。
对比图对极性/非极性两侧采用相同色标，仅在绘图时平铺 3x3，不重新计算超胞。
...
上游大型 SCF 输出；每个案例的 `run/README.md` 都记录了这一边界和来源。保留
