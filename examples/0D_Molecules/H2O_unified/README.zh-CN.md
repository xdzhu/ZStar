# H2O：Unified 与 Cartesian 响应对照

H2O 在所附 PBE 基组、20 埃立方盒内重新优化；Gamma 采样、100 Ry、SCF 1e-8。

两套流程使用相同结构、基组、SCF 参数和位移长度。笛卡尔中心差分为
12 个位移，Unified 为 6 个，分别加一个参考 SCF。
...
```bash
python examples/Benchmarks/run_control.py H2O
```
