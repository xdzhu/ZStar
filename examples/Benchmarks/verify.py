...
def verify(case, scheme='shared', name=None):
    ...
    if checksums.is_file():
        for filename, expected in json.loads(checksums.read_text()).items():
            digest = hashlib.sha256()
            with (case/filename).open('rb') as handle:
                for block in iter(lambda: handle.read(1024*1024), b''):
                    digest.update(block)
            if digest.hexdigest() != expected:
                raise ValueError(f'Archive checksum mismatch: {case.name}/{filename}')
    ...
    with tempfile.TemporaryDirectory(prefix='zstar-shared-check-') as temporary:
        ...
        if name in ('SiC', 't_HfO2', 'alpha_In2Se3'):
            benchmark = json.loads((INDEX/'benchmark_summary.json').read_text())
            key = {'SiC': 'sic', 't_HfO2': 'hfo2', 'alpha_In2Se3': 'in2se3'}[name]
            ...
            entry = json.loads((INDEX/'four_new_benchmarks.json').read_text())['cases'][name]
        ...
        if result['dimension'] == 0:
            ...
            # Recollection produces this file; historical aliases may retain
            # different rounding and must not participate in implicit discovery.
            born = read_born_data(copied/'BEC.dat', natoms=len(modes.masses_amu))
...
if __name__ == '__main__':
    ...
    completed = [verify(case_path(root, name), name=name) for name in ('SiC', 't_HfO2', 'alpha_In2Se3')]
    if (case_path(root, 'alpha_In2Se3')/'results/shared-mesh88/shared_response_result.json').is_file():
        completed.append(verify(case_path(root, 'alpha_In2Se3'), 'shared-mesh88', name='alpha_In2Se3'))
    for name in ('hBN', 'MoS2', 'H2O', 'CH4'):
        for scheme in ('unified', 'cartesian'):
            completed.append(verify(case_path(root, name), scheme, name=name))
    for scheme in ('unified-mesh112', 'cartesian-mesh112'):
        completed.append(verify(case_path(root, 'MoS2'), scheme, name='MoS2'))
