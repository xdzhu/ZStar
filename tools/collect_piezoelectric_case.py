#!/usr/bin/env python3
"""Collect and fit a completed ABACUS + PYATB strain-response case.
...
DEFAULT_ZERO_ABSOLUTE_TOLERANCES = {
    # A symmetry-forbidden proper piezoelectric tensor has no nonzero norm
    # against which to define a relative residual.  Use a fixed, unit-bearing
    # numerical-noise gate instead of dividing by a near-zero fitted tensor.
    "piezoelectric_proper": 1.0e-3,  # C/m^2
}

DEFAULT_NONZERO_ABSOLUTE_TOLERANCES = {
    # Frozen production tensor-stability target from the calculation protocol.
...
def _symmetry_response_audit(
    ...
    *,
    ...
    zero_tolerances = dict(DEFAULT_ZERO_ABSOLUTE_TOLERANCES)
    if zero_absolute_tolerances is not None:
        zero_tolerances.update(
            {str(name): float(value) for name, value in zero_absolute_tolerances.items()}
        )
    ...
    for quantity_name, (output_kind, major_symmetric) in quantity_specs.items():
        ...
        forbidden_max = float(np.max(np.abs(difference)))
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0e-3,
        help="relative Frobenius gate for the post-fit symmetry audit (default: 1e-3)",
    )
    parser.add_argument(
        "--acoustic-gauge",
    ...
    if missing_gap or metallic:
        ...
        raise ValueError(
            "piezoelectric collection requires every reference/strain stage to be insulating; "
            + "; ".join(details)
        )
    ...
    summary = {
        ...
        # but that provenance must not silently redefine the fixed protocol.
