...
The shared route instead records its joint data in `response_fit.json`
and must not be claimed as a newly DFT-validated molecular benchmark solely
from the bulk/slab shared tests. Check `apt.json`, including the symmetry-expanded raw translational
